...
export function initSocketServer(io: SocketIOServer) {
  io.on("connection", (socket: Socket) => {
    ...
    socket.on("room:create", (payload: { name: string; username: string; avatar: string; userId: string; passage?: any; difficulty?: string }) => {
      ...
      const room: RoomData = {
        id: code,
        code,
        name: payload.name || `${payload.username}'s Grand Prix`,
    ...
    socket.on("room:join", (payload: { code: string; username: string; avatar: string; userId: string }) => {
    ...
    socket.on("race:finish", (data: { finalWpm: number; accuracy: number; duration: number }) => {
      if (!currentRoomCode) return;
      const room = rooms.get(currentRoomCode);
      if (!room || room.status !== "RACING" || !room.players[socket.id]) return;
...
function startBotsSimulation(room: RoomData, io: SocketIOServer) {
  ...
  bots.forEach((bot) => {
    ...
    if (!room.botIntervals) room.botIntervals = [];
    room.botIntervals.push(botInterval);
  });
}

function sanitizeRoom(room: RoomData) {
  return {
    id: room.id,
    code: room.code,
    name: room.name,
